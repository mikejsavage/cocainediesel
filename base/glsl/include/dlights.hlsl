...
void AddDynamicLights( uint count, int tile_index, float3 position, float3 normal, float3 viewDir, inout float3 lambertlight, inout float3 specularlight ) {
	for( uint i = 0; i < count; i++ ) {
		...
		specularlight += dlight_color * max( 0.0f, 1.0f - dist / radius ) * SpecularLight( normal, lightdir, viewDir, u_MaterialProperties[ 0 ].shininess ) * u_MaterialProperties[ 0 ].specular;
	}
}

#endif // #if APPLY_DYNAMICS
