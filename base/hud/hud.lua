...
local function DrawBombProgress( state )
	if state.bomb_progress ~= 0 then
		...
		cd.text( text, state.viewport_width * 0.5, y + height * 0.5, message )
	end
end
...
return function( state )
	...
	if state.match_state < MatchState_PostMatch then
		...
		cd.drawObituaries( state.viewport_width - 10, 2, state.viewport_width / 10, state.viewport_width / 10, state.viewport_height / 20, state.viewport_width / 95 )
	end
