...
void CG_EBImpact( const vec3_t pos, const vec3_t dir, int surfFlags, int team ) {
	...
	S_StartFixedSound( CG_MediaSfx( cgs.media.sfxElectroboltHit ), pos, CHAN_AUTO, cg_volume_effects->value, ATTN_STATIC );
}
...
void CG_RocketExplosionMode( const vec3_t pos, const vec3_t dir, float radius, int team ) {
	...
	S_StartFixedSound( CG_MediaSfx( cgs.media.sfxRocketLauncherHit ), pos, CHAN_AUTO, cg_volume_effects->value, ATTN_DISTANT );
}
...
void CG_BladeImpact( const vec3_t pos, const vec3_t dir ) {
	...
	if( trace.surfFlags & SURF_FLESH ||
		...
		S_StartFixedSound( CG_MediaSfx( cgs.media.sfxBladeFleshHit[(int)( random() * 3 )] ), pos, CHAN_AUTO,
...
void CG_GrenadeExplosionMode( const vec3_t pos, const vec3_t dir, float radius, int team ) {
	...
	S_StartFixedSound( CG_MediaSfx( cgs.media.sfxGrenadeExplosion ), pos, CHAN_AUTO, cg_volume_effects->value, ATTN_DISTANT );
}
