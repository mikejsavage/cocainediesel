...
static void *QFT_AllocGlyphs( qfontface_t *qfont, wchar_t first, unsigned int count ) {
	return Mem_Alloc( ftlibPool, count * sizeof( qftglyph_t ) );
}
...
static qftfallback_t *QFT_GetFallbackFace( qfontfamily_t *qfamily, unsigned int size ) {
	...
	fallback = ( qftfallback_t * ) Mem_Alloc( ftlibPool, sizeof( qftfallback_t ) );
...
static void QFT_RenderString( qfontface_t *qfont, const char *str ) {
	...
	for( ; ; ) {
		...
		if( bitmapHeight > qftGlyphTempBitmapHeight ) {
			...
			qftGlyphTempBitmap = ( uint8_t * ) Mem_Realloc( qftGlyphTempBitmap, FTLIB_FONT_MAX_IMAGE_WIDTH * qftGlyphTempBitmapHeight );
		}
...
static qfontface_t *QFT_LoadFace( qfontfamily_t *family, unsigned int size ) {
	...
	qttf = ( qftface_t * ) Mem_Alloc( ftlibPool, sizeof( *qttf ) );
	qttf->ftsize = ftsize;
	...
	// store font info
	qfont = ( qfontface_t * ) Mem_Alloc( ftlibPool, sizeof( qfontface_t ) );
	qfont->family = family;
	qfont->size = size;
...
static bool QFT_LoadFamily( const char *fileName, uint8_t *data, size_t dataSize, bool verbose, bool fallback ) {
	...
	qftfamily = ( qftfamily_t * ) Mem_Alloc( ftlibPool, sizeof( qftfamily_t ) );
	qftfamily->ftface = ftface;
	qftfamily->file = data;

	qfamily = ( qfontfamily_t * ) Mem_Alloc( ftlibPool, sizeof( qfontfamily_t ) );
	qfamily->numFaces = 0;
...
static void QFT_LoadFamilyFromFile( const char *name, const char *fileName, bool verbose, bool fallback ) {
	...
	buffer = ( uint8_t * )Mem_Alloc( ftlibPool, length );
...
void FTLIB_InitSubsystems() {
	...
	qftGlyphTempBitmap = ( uint8_t * ) Mem_Alloc( ftlibPool, FTLIB_FONT_MAX_IMAGE_WIDTH * QFT_GLYPH_BITMAP_HEIGHT_INCREMENT );
