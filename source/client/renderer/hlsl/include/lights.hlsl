...
void AddLights( uint count, int tile_index, float3 position, float3 normal, float3 viewDir, inout float3 lambertlight, inout float3 specularlight ) {
	for( uint i = 0; i < count; i++ ) {
		...
		float3 color = sRGBToLinear( light.color ).xyz;

		float intensity = LIGHT_CUTOFF * light.radius * light.radius;
		float dist = distance( position, light.origin );
		float3 lightdir = normalize( position - light.origin - normal ); // - normal to prevent 0,0,0
		...
		specularlight += color * max( 0.0f, 1.0f - dist / light.radius ) * SpecularLight( normal, lightdir, viewDir, u_MaterialProperties[ 0 ].shininess ) * u_MaterialProperties[ 0 ].specular;
	}
}
