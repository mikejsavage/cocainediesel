...
void DrawTextBaseline( const Font * font, float pixel_size, Span< const char > str, float x, float y, Vec4 color, Optional< Vec4 > border_color ) {
	...
	for( size_t i = 0; i < str.n; i++ ) {
		...
		if( Area( glyph->bounds ) > 0.0f ) {
			...
			Vec2 bl = Vec2( x, y ) + pixel_size * flip_y * ( glyph->bounds.mins - font->metadata.glyph_padding );
			Vec2 tr = Vec2( x, y ) + pixel_size * flip_y * ( glyph->bounds.maxs + font->metadata.glyph_padding );

			bg->PrimReserve( 6, 4 );
			bg->PrimRectUV( bl, tr, glyph->padded_uv_bounds.mins, glyph->padded_uv_bounds.maxs, IM_COL32_WHITE );
		}
...
void DrawText( const Font * font, float pixel_size, Span< const char > str, Alignment align, float x, float y, Vec4 color, Optional< Vec4 > border_color ) {
	...
	switch( align.y ) {
		...
		case YAlignment_Middle: y += ( font->metadata.ascent + font->metadata.descent ) * pixel_size * 0.5f; break;
