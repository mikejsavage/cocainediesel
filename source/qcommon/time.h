...
template< typename T >
constexpr Time Milliseconds( T ms ) {
	return { u64( ms * GGTIME_FLICKS_PER_SECOND / 1000 ) };
}

template< typename T >
constexpr Time Seconds( T secs ) {
	return { u64( secs * GGTIME_FLICKS_PER_SECOND ) };
}

template< typename T >
constexpr Time Minutes( T mins ) {
	return { u64( mins * GGTIME_FLICKS_PER_SECOND * 60 ) };
}
